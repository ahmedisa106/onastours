...
class TripRepository
{
    ...
    {
        $trips = Trip::where('id', '!=', $exclude)->where('is_home_page', 1)->with(['categories', 'categories.translations', 'translations'])->orderBy('created_at', 'desc')->take($limit)->get();
        return $trips;
    }

    public function UpdateHomeTrips($id, $active)
    {
        $trip = $this->find($id);
        $trip->update(['is_home_page' => $active]);
        return $trip;
