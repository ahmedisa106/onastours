...
<section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><a href="{{url('commonmodule/')}}"><i class="fa fa-home"></i> {{__('VideoModule::vidcateg.homepage')}}</a></li>
      <li class="active">{{__('VideoModule::vidcateg.pagetitle')}}</li>
    </ol>
</section>
@endsection
...
<section class="content">
  ...
  <div class="box box-info">
    ...
    @endif
    <!-- /.box-header -->
    <form class="form-horizontal" action="{{url('admin-panel/videos/videocategory')}}/{{$vidcateg->id}}" method="POST">
      {{ method_field('PUT') }}
      {{ csrf_field() }}
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <input data-validation="length alphanumeric" data-validation-length="min4" type="text" autocomplete="off" class="form-control"
                      placeholder="Write Title" value={{$vidcateg->translate(''.$lang->lang)->title}} name="{{$lang->lang}}[title]" required data-validation="alphanumeric">
                  </div>
                </div>
              </div>
              @endforeach
