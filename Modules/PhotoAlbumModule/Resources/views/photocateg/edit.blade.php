...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        <li><a href="{{url('commonmodule/')}}"><i class="fa fa-home"></i> {{__('photoalbummodule::photocateg.homepage')}}</a></li>
        <li class="active">{{__('photoalbummodule::photocateg.pagetitle')}}</li>
    </ol>
</section>
@endsection
...
<section class="content">
    ...
    <div class="box box-info">
        ...
        @endforeach @endif
        <!-- /.box-header -->
        <form class="form-horizontal" action="{{url('admin-panel/photos/photocategory')}}/{{$category->id}}" method="POST">
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane @if($loop->first) active @endif" id="{{ $lang->display_lang }}">
                                <div class="form-group">
                                    {{-- title --}}
                                    <label class="control-label col-sm-2" for="title">{{__('photoalbummodule::photocateg.pagetitle')}} ({{ $lang->display_lang }}):</label>
                                    <div class="col-sm-8">
                                        <input data-validation="length alphanumeric" data-validation-length="min4" type="text" autocomplete="off" class="form-control"
                                        name="{{$lang->lang}}[title]" @if($loop->first) required @endif data-validation="alphanumeric" value={{ ValueOf($category,$lang,'title') }}>
                                    </div>
                                </div>
                            </div>
                            @endforeach
